...
define goCross
	...
	printf "$(OK_CLR)$(MSG_PRFX) Building binary for [$(1)/$(2)]$(MSG_SFX)$(NO_CLR)\n";      \
	printf "$(INFO_CLR)\
...
clean-bin: ## to clean generated artifacts.
	@echo "$(WARN_CLR)$(MSG_PRFX) 🧹 Cleaning up binaries$(MSG_SFX)$(NO_CLR)"
	@rm -rf $(ARTIFACTS_PATH) 2>&1

clean-version: ## to remove version file.
	@echo "$(WARN_CLR)$(MSG_PRFX) 🧹 Cleaning up version file$(MSG_SFX)$(NO_CLR)"
	@rm -f  $(VERSION_FILE)
...
go-generate: ## to generate Go related files.
	@echo "$(OK_CLR)$(MSG_PRFX) Generating files via Go generate$(MSG_SFX)$(NO_CLR)"
...
install: ## to install the generated binary.
	@echo "$(OK_CLR)$(MSG_PRFX) Installing generated binary$(MSG_SFX)$(NO_CLR)"
...
kill: ## to send a kill signal to the running process of the binary.
	@echo "$(WARN_CLR)$(MSG_PRFX) Sending kill signal $(args)$(MSG_SFX)$(NO_CLR)"
	@pkill $(args) $(notdir $(TARGET_BINARY)) > /dev/null 2>&1

run: ## to run the generated binary, and build a new one if not existed.
	@echo "$(OK_CLR)$(MSG_PRFX)  🏃 Running generated binary$(MSG_SFX)$(NO_CLR)"
	@if [ ! -f $(TARGET_BINARY) ] ; then $(MAKE) build; fi
	@$(TARGET_BINARY) $(args) 2>&1

uninstall: ## to uninstall generated binary.
	@echo "$(WARN_CLR)$(MSG_PRFX) Uninstalling generated binary$(MSG_SFX)$(NO_CLR)"
...
version:  ## to get the current version.
	@echo "$(INFO_CLR)$(MSG_PRFX) Current tagged version$(MSG_SFX)$(NO_CLR)"
	@echo "$(OK_CLR)$(MSG_PRFX) $(VERSION) $(NO_CLR)"
