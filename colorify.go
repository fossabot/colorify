...
// SetForeground color.
func (c *Colorable) SetForeground(red, green, blue int) *Colorable {
	c.style.Foreground = &ColorValue{
		Red:   red,
		Green: green,
		Blue:  blue,
	}
...
// SetBackground color.
func (c *Colorable) SetBackground(red, green, blue int) *Colorable {
	c.style.Background = &ColorValue{
		Red:   red,
		Green: green,
		Blue:  blue,
	}
...
// SetFont effects.
func (c *Colorable) SetFont(fontEffects ...FontEffect) *Colorable {
	effects := make([]FontEffect, len(fontEffects))

	for _, fontEffect := range fontEffects {
		effects = append(effects, fontEffect)
	}

	c.style.Font = effects
...
// Sample of available colors.
func (c *Colorable) Sample() string {
	sample := make([]string, 0)
	for colorIndex := 0; colorIndex <= 255; colorIndex++ {
		red := (colorIndex + 5) % 255
		green := colorIndex * 3 % 255
		blue := 255 - colorIndex
